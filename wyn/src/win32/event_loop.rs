...
impl EventLoop<'_> {
    ...
    unsafe fn internal_wndproc_impl(
        ...
        //         "|^ -------- EVENT"
        //     };
        //     log::event(label, hwnd, umsg, wparam, lparam);

        //     unsafe { sys::SetLastError(error) };
        // };
        // _debug(true);
        // let _defer = defer::defer(|| _debug(false));
        // ---------------------------------------------------------------- //
        ...
        if let Some(events) = &EVENTLOOP {
            match umsg {
                ...
                sys::WM_NCCALCSIZE => {
                    //eprintln!("NC-CALCSIZE: W: {wparam}");
                    // if wparam == (sys::TRUE as sys::WPARAM) {
                    //     let _res = sys::DefWindowProcW(hwnd, umsg, wparam, lparam);
                    //     return (sys::WVR_ALIGNBOTTOM
                    //         | sys::WVR_ALIGNRIGHT
                    //         | sys::WVR_VREDRAW
                    //         | sys::WVR_HREDRAW) as sys::LRESULT;
                    // }
                }
                // https://learn.microsoft.com/en-us/windows/win32/winmsg/wm-windowposchanging
                sys::WM_WINDOWPOSCHANGING => {
                    if let Some(wpos) = (lparam as *mut sys::WINDOWPOS).as_mut() {
                        wpos.flags |= sys::SWP_NOCOPYBITS;
                    }
                    return 0;
                }
