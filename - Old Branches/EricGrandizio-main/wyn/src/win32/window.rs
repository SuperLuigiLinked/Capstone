...
impl Window {
    ...
    const WS_EX_DEFAULT: sys::WINDOW_EX_STYLE = sys::WS_EX_APPWINDOW; // | sys::WS_EX_LAYERED;
...
impl Window {
    ...
    pub fn request_redraw(&self, _events: &EventLoop) {
        let handle = self.handle();

        let _invalidate = move || {
            let _res = unsafe { sys::InvalidateRect(handle, null(), sys::FALSE) };
        };
        ...
        let _redraw = move || {
            ...
            let _res = unsafe { sys::RedrawWindow(handle, null(), 0, flags) };
