...
impl DeviceMemory {
    ...
    pub fn new(
        ...
        let res = utils::find_memory_type(physical.memory_types(), memory_bits, memory_props);
...
impl<'a> MappedMemory<'a> {
    ...
    /// Copies the given bytes to this memory at the given offset, then returns the offset past the end of the copied bytes.
    pub fn copy_to(&mut self, offset: vk::DeviceSize, bytes: &[u8]) -> vk::DeviceSize {
        let old_offset = usize::try_from(offset).unwrap();
        let new_offset = old_offset + bytes.len();

        if !bytes.is_empty() {
            let mem_slice = &mut self.bytes_mut()[old_offset..new_offset];
            mem_slice.copy_from_slice(bytes);
        }

        vk::DeviceSize::try_from(new_offset).unwrap()
    }
}
